...
↓ 语法分析+语义分析（yacc）
↓ 生成中间代码（LLVM IR ）
↓ 中间代码优化（可选）——————————————>后期选择opt组件对LLVM IR进行优化
↓ 寄存器分配 + 栈帧分配
↓ 生成目标代码（x86汇编）——————————————>用LLC组件将LLVM IR转换成目标平台汇编代码，x86或者RISC-V
...
- 这条函数定义的产生式中decl-specifier-seq可选是为了满足A();             // 构造函数：无返回类型 
   													 ~A();            // 析构函数：无返回类型

- 目前不考虑构造函数、析构函数。
- 所以decl-specifier-seq一定要有
